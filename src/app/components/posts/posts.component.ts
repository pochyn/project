...
export class PostsComponent implements OnInit {
 ...
 ngOnInit() {
      ...
      this.sites = this.postsColSite.snapshotChanges()
          ...
          }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.gazeta_type && post.data.checked_gazeta && !post.data.archieved_g));       
      //site tab paginator
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.gazeta_type && !post.data.read && !post.data.archieved_g));
   ...
   this.lviv = this.postsColLviv.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.gazeta_type && post.data.read && !post.data.checked_gazeta && !post.data.archieved_g));
   //gazeta tab paginator
