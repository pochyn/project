...
export class SoloRegionsComponent implements OnInit {
 ...
 ngOnInit() {
      ...
      this.sites = this.postsColSite.snapshotChanges()
          ...
          }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.regions_type && post.data.checked_regions && !post.data.archieved_solo));       
      //site tab paginator
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.regions_type && !post.data.read && !post.data.archieved_solo));
   ...
   this.lviv = this.postsColLviv.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.regions_type && post.data.read && !post.data.checked_gazeta && !post.data.archieved_solo));
   //gazeta tab paginator
