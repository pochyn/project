...
@Component({
  selector: 'app-solo-regions',
  templateUrl: './solo-regions.component.html',
  styleUrls: ['./solo-regions.component.css']
})
export class SoloRegionsComponent implements OnInit {
 ...
 ngOnInit() {
      ...
      this.sites = this.postsColSite.snapshotChanges()
          ...
          }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.regions_type && post.data.checked_regions && !post.data.archieved_solo && !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));       
      //site tab paginator
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.regions_type && !post.data.read && !post.data.archieved_solo && !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));
