...
@Component({
  selector: 'app-solo-archieve',
  templateUrl: './solo-archieve.component.html',
  styleUrls: ['./solo-archieve.component.css']
})
export class SoloArchieveComponent implements OnInit {
    ...
    .map(actions => {
      return actions.map(a => {
        ...
        return { id, data };
                                  ...
                                  && post.data.archieved_solo && !post.data.delete_solo));
...
deletePost(postId, postdata) {
  ...
  if (postdata.delete_or && postdata.delete_kv && (postdata.delete_gr || postdata.delete_solo)) {
