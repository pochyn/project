...
@Component({
  selector: 'app-solo-comments',
  templateUrl: './solo-comments.component.html',
  styleUrls: ['./solo-comments.component.css']
})
export class SoloCommentsComponent implements OnInit {
 ...
 displayedColumns = ['data.date_modified', 'data.comments'];
     ...
     .map(actions => {
       return actions.map(a => {
         ...
         return { id, data };
                                   ...
                                   && post.data.comments != '' && !post.data.archieved_solo));
