...
export class SitesComponent implements OnInit {
   ...
   ngOnInit() {
        ...
        this.sites = this.postsColSite.snapshotChanges()
            ...
            }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.site_type && post.data.checked_site && !post.data.archieved_g && !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));       
        //site tab paginator
     ...
     this.posts = this.postsColGaz.snapshotChanges()
       ...
       }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.site_type && !post.data.read && !post.data.archieved_g && !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));
