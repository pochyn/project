 ...
 ngOnInit() {
  ...
  queryRef.get().then((snapShot) => {
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.branch == br && post.data.regions_type && !post.data.read && !post.data.archieved_kv));
   ...
   this.lviv = this.postsColLviv.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.branch == br && post.data.regions_type && post.data.read && !post.data.checked_regions && !post.data.archieved_kv));
   //gazeta tab paginator
