...
import {AllShowMediaComponent} from '../../all-show-media/all-show-media.component';
...
@Component({
  selector: 'app-kv-media',
  templateUrl: './kv-media.component.html',
  styleUrls: ['./kv-media.component.css']
})
export class KvMediaComponent implements OnInit {
...
ngOnInit() {
  ...
  this.posts = this.postsColGaz.snapshotChanges()
    ...
    }).map(posts => posts.filter(post => (!post.data.read) && (post.data.mediaplan_gazeta || post.data.mediaplan_lviv  || post.data.mediaplan_regions || post.data.mediaplan_site) && (!post.data.archieved_kv)));
  ...
  this.sites = this.postsColSite.snapshotChanges()
      ...
      }).map(posts => posts.filter(post => (post.data.read) && (post.data.checked_gazeta || post.data.checked_lviv || post.data.checked_regions || post.data.checked_site ) && (post.data.mediaplan_gazeta || post.data.mediaplan_lviv  || post.data.mediaplan_regions || post.data.mediaplan_site) && (!post.data.archieved_kv)));       
  //site tab paginator
