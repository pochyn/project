 ...
 ngOnInit() {
  ...
  queryRef.get().then((snapShot) => {
      ...
      this.sites = this.postsColSite.snapshotChanges()
          ...
          }).map(posts => posts.filter(post => post.data.branch == br && post.data.site_type && post.data.checked_site && !post.data.archieved_kv));       
      //site tab paginator
      console.log(br)
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.branch == br && post.data.site_type && !post.data.read && !post.data.archieved_kv));
   ...
   this.lviv = this.postsColLviv.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.branch == br && post.data.site_type && post.data.read && !post.data.checked_site && !post.data.archieved_kv));
   //gazeta tab paginator
