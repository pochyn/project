...
@Component({
  selector: 'app-kv-archieve',
  templateUrl: './kv-archieve.component.html',
  styleUrls: ['./kv-archieve.component.css']
})
export class KvArchieveComponent implements OnInit {
      ...
      .map(actions => {
        return actions.map(a => {
          ...
          return { id, data };
                                    ...
                                    && post.data.archieved_kv && !post.data.delete_kv));
  ...
  deletePost(postId, postdata) {
    this.afs.doc('posts/'+postId).update({delete_kv: true});
    if (postdata.delete_or && postdata.delete_g && postdata.delete_gr) {
