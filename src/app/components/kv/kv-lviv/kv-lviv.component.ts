 ...
 ngOnInit() {
  ...
  queryRef.get().then((snapShot) => {
      ...
      this.sites = this.postsColSite.snapshotChanges()
          ...
          }).map(posts => posts.filter(post => (post.data.branch == br && post.data.lviv_type && post.data.checked_lviv && !post.data.archieved_kv&& !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site) || (post.data.nascrizna == true && this.nascriznyy) )));       
      //site tab paginator
      console.log(br)
   ...
   this.lviv = this.postsColLviv.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.branch == br && post.data.lviv_type && post.data.read_kv && !post.data.checked_lviv && !post.data.archieved_kv&& !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));
   //gazeta tab paginator
