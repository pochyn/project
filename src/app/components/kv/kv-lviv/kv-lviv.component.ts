 ...
 ngOnInit() {
  ...
  queryRef.get().then((snapShot) => {
      ...
      this.sites = this.postsColSite.snapshotChanges()
          ...
          }).map(posts => posts.filter(post => post.data.branch == br && post.data.lviv_type && post.data.checked_lviv && !post.data.archieved_kv));       
      //site tab paginator
      console.log(br)
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.branch == br && post.data.lviv_type && !post.data.read_kv && !post.data.archieved_kv));
   ...
   this.lviv = this.postsColLviv.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.branch == br && post.data.lviv_type && post.data.read_kv && !post.data.checked_lviv && !post.data.archieved_kv));
   //gazeta tab paginator
