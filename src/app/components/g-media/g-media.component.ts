...
@Component({
  selector: 'app-g-media',
  templateUrl: './g-media.component.html',
  styleUrls: ['./g-media.component.css']
})
export class GMediaComponent implements OnInit {
...
ngOnInit() {
  ...
  this.posts = this.postsColGaz.snapshotChanges()
    ...
    }).map(posts => posts.filter(post => (!post.data.read) && (post.data.mediaplan_gazeta || post.data.mediaplan_lviv  || post.data.mediaplan_regions || post.data.mediaplan_site) && (!post.data.archieved_g)));
