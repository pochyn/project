...
@Component({
  selector: 'app-archieve-or-gazeta',
  templateUrl: './archieve-or-gazeta.component.html',
  styleUrls: ['./archieve-or-gazeta.component.css']
})
export class ArchieveOrGazetaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.posts = this.postsColGaz.snapshotChanges()
      ...
      }).map(posts => posts.filter(post => post.data.gazeta_type
                                    && post.data.archieved_or && !post.data.delete_or));
  ...
  undoArchieve(postId, postdata){
    this.afs.doc('posts/'+postId).update({archieved_or: false});
  }
  
  //delete (not archive)
  deletePost(postId, postdata) {
    this.afs.doc('posts/'+postId).update({delete_or: true});
    if (postdata.delete_g && postdata.delete_kv && postdata.delete_gr) {
