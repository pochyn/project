...
@Component({
  selector: 'app-gazeta-media',
  templateUrl: './gazeta-media.component.html',
  styleUrls: ['./gazeta-media.component.css']
})
export class GazetaMediaComponent implements OnInit {
 ...
 ngOnInit() {
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => (!post.data.read) && (post.data.mediaplan_gazeta || post.data.mediaplan_lviv  || post.data.mediaplan_regions || post.data.mediaplan_site) && (!post.data.archieved_or)));
