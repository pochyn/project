...
@Component({
  selector: 'app-gr-site',
  templateUrl: './gr-site.component.html',
  styleUrls: ['./gr-site.component.css']
})
export class GrSiteComponent implements OnInit {
 ...
 ngOnInit() {
  ...
  element.classList.add("hidden")
   ...
   this.posts = this.postsColGaz.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.checked && post.data.site_type && !post.data.read && !post.data.archieved_gr&& !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));
   ...
   this.sites = this.postsColSite.snapshotChanges()
       ...
       }).map(posts => posts.filter(post => post.data.checked && post.data.site_type && post.data.checked_site && !post.data.archieved_gr&& !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));       
   //site tab paginator
   ...
   this.lviv = this.postsColLviv.snapshotChanges()
     ...
     }).map(posts => posts.filter(post => post.data.checked && post.data.site_type && post.data.read && !post.data.checked_site && !post.data.archieved_gr&& !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));
   //gazeta tab paginator
