...
import { ShowTopicOrSiteComponent } from '../show-topic-or-site/show-topic-or-site.component';
...
@Component({
  selector: 'app-post-or-site',
  templateUrl: './post-or-site.component.html',
  styleUrls: ['./post-or-site.component.css']
})
export class PostOrSiteComponent implements OnInit {
   ...
   queryRef2.get().then((snapShot) => {
       ...
       queryRef1.get().then((snapShot) => {
           ...
           this.sites = this.postsColSite.snapshotChanges()
               ...
               }).map(posts => posts.filter(post => ( post.data.site_type && post.data.checked_site && !post.data.archieved_or  && !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site) || (post.data.nascrizna == true && this.nascriznyy && post.data.checked_site) )));              
               //site tab paginator
    ...
    this.posts = this.postsColGaz.snapshotChanges()
      ...
      }).map(posts => posts.filter(post => post.data.author == this.auth.currentUserId && post.data.site_type && !post.data.read && !post.data.archieved_or  && !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));
  ...
  showPost(postid, postdata){
    this.dialogRef.open(ShowTopicOrSiteComponent, {
     width: '90vw',
      data: {
        postId: postid,
        postdata: postdata,
