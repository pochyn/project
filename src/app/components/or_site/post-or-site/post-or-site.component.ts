...
export class PostOrSiteComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.posts = this.postsColApp.snapshotChanges()
      ...
      }).map(posts => posts.filter(post => post.data.site_type && post.data.checked_site &&
                         post.data.site_type && !post.data.archieved_or));
    ...
    this.rej = this.postsColRej.snapshotChanges()
        ...
        }).map(posts => posts.filter(post => post.data.read && !post.data.checked_site
                                      && post.data.site_type && !post.data.archieved_or));       
    //site tab paginator
